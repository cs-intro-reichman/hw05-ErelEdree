...
public class GameOfLife {
	...
	public static int[][] evolve(int[][] board) {
		// initialising a new board that we will write to, so we can read only from
		// current board
		int rows = board.length;
		int cols = board[0].length;
		int[][] updatedBoard = new int[rows][cols];

		// evolve each cell in the board
		for (int i = 1; i < board.length - 1; i++) {
			for (int j = 1; j < board[0].length - 1; j++) {
				updatedBoard[i][j] = cellValue(board, i, j);
			}
		}
		return updatedBoard;
	}
	...
	public static int cellValue(int[][] board, int i, int j) {
		//// Replace the following statement with your code.
		int cell = board[i][j];
		int cell_count = count(board, i, j);
		if (cell == 1) {
			if (cell_count < 2) {
				return 0;
			}
	...
	public static void show(int[][] board) {
		...
		double halfWidth = 0.45; // Slightly less than half to create a space between squares
		for (int i = 0; i < rows; i++) {
			for (int j = 0; j < cols; j++) {
				int color = 255 * (1 - board[i][j]);
				StdDraw.setPenColor(color, color, color);
				StdDraw.filledRectangle(j + 0.5, rows - i - 0.5, halfWidth, halfWidth);
			}
		}
		StdDraw.show();
		StdDraw.pause(500);
